...
export const Form = () => {
    const { dataRegister, profiles, updateProfiles, profile, updateProfile, result, setResult } =
    ...
    useEffect(() => {
        ...
        if (isSubmitted)
            toast.success('Cliente creado. Haz clic en el botón Editar para modificar.', {
                position: 'top-center',
                style: {
                    maxWidth: '500px',
                },
    ...
    const isAllChecked = dataAuthorization && risksAuthorization && conditionsAuthorization
    return (
        <>
            <Toaster
            ...
            {searchClient && (
                <div className="container">
                    <ValidateClient setSearchClient={setSearchClient} reset={reset} setIsEdit={setIsEdit} />
                </div>
            )}
            {!searchClient && <Title text={'Información Preliminar'} attribute={'title__large information'} />}
            {!searchClient && (
                <form onSubmit={handleSubmit(onSubmit)}>
                    <fieldset disabled={isSubmitted}>
                        <PersonalData options={{ register, watch, errors }} />
                        <LaboralData
                            options={{ register, watch, control, errors, readOnly: isSubmitted, getDataClient }}
                        />
                        <EconomicData
                            options={{ register, control, errors, watch, readOnly: isSubmitted, getDataClient }}
                        />
                        <RequestData
                            options={{ register, control, watch, errors, readOnly: isSubmitted, getDataClient }}
                        />
            ...
            {result ? <Loader /> : Object.keys(profile).length > 0 && <Result clientID={client?.id} />}
        </>
    )
}
